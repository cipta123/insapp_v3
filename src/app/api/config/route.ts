...
export async function GET() {
  try {
    ...
    const maskedConfig = {
      instagram: {
        ...config.instagram,
        accessToken: config.instagram.accessToken ? '••••••••' + config.instagram.accessToken.slice(-8) : '',
        appSecret: config.instagram.appSecret ? '••••••••' + config.instagram.appSecret.slice(-4) : '',
        webhookVerifyToken: config.instagram.webhookVerifyToken ? '••••••••' + config.instagram.webhookVerifyToken.slice(-4) : ''
      },
      whatsapp: {
        ...config.whatsapp,
        accessToken: config.whatsapp.accessToken ? '••••••••' + config.whatsapp.accessToken.slice(-8) : '',
        appSecret: config.whatsapp.appSecret ? '••••••••' + config.whatsapp.appSecret.slice(-4) : '',
        webhookVerifyToken: config.whatsapp.webhookVerifyToken ? '••••••••' + config.whatsapp.webhookVerifyToken.slice(-4) : ''
      }
    }
