...
export async function GET(request: NextRequest) {
  try {
    ...
    // Temporary fix: Return empty array if WhatsApp model not available yet
    try {
      ...
      console.log('⚠️ WHATSAPP_API: WhatsApp model not available yet, returning empty array')
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Temporary fix: Handle WhatsApp model not available yet
    try {
      ...
      console.log('⚠️ WHATSAPP_API: WhatsApp model not available yet, simulating success')
      return NextResponse.json({ 
        id: 'temp_' + Date.now(),
        messageId,
